...
public abstract class BaseRepositoriesTests : IAsyncLifetime
{
    ...
    protected async Task ClearDatabaseAsync()
    {
        try
        {
            ...
            await freshContext.Database.ExecuteSqlRawAsync(
                "DELETE FROM sqlite_sequence WHERE name IN ('Prontuarios', 'FamiliaUsuarios', 'TecnicoCras', 'Familias', 'Usuarios', 'Tecnicos', 'Cras');");
        }
        catch (Exception ex)
        {
